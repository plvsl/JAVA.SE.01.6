...
public class NotePad {
    ...
    private static final double EXPANSION_COEFF = 1.5;
    private Note notes[];
    ...
    void addNote(Note noteToAdd) {
        ...
        if (!marker) {
            Note temp[] = Arrays.copyOf(notes, (int) (notes.length * EXPANSION_COEFF));
    ...
    void removeNote(int index) {
        ...
        if (checkElement(index)) {
            ...
            if (notes.length > actualSize * EXPANSION_COEFF) {
                temp = new Note[(int) (actualSize * EXPANSION_COEFF)];
            }
            else {
                temp = new Note[actualSize];
            }
            System.arraycopy(notes, 0, temp, 0, index);
            System.arraycopy(notes, index + 1, temp, index, actualSize - index);
            notes = temp;
        } else {
